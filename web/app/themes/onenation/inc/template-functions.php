 ...
 */
if ( ! function_exists( 'fashion_is_frontpage' ) ) {

	function fashion_is_frontpage() {
		return ( is_front_page() || is_home() );
	}
}


if ( ! function_exists( 'fashion_page_enable_breadcrumb' ) ) {
	/**
	 * @return bool
	 */
	function fashion_page_enable_breadcrumb() {

		if ( ! is_page() ) {
			return true;
		}

		$check = fashion_get_metabox( get_the_ID(), 'osf_enable_breadcrumb', true );
...
if ( ! function_exists( 'fashion_get_query' ) ) {
	 ...
	 * @return WP_Query
	 */
	function fashion_get_query( $args ) {
		global $wp_query;
		$default  = array(
			'post_type' => 'post',
		);
...
if ( ! function_exists( 'fashion_get_placeholder_image' ) ) {

	/**
	 * @return string
	 */
	function fashion_get_placeholder_image() {
		return get_parent_theme_file_uri( '/assets/images/placeholder.png' );
	}

}

if (!function_exists('fashion_is_elementor_activated')) {
    function fashion_is_elementor_activated() {
        return function_exists('elementor_load_plugin_textdomain');
    }
}

if ( ! function_exists( 'fashion_is_osf_framework_activated' ) ) {
	/**
	 * Query WooCommerce activation
	 */
	function fashion_is_osf_framework_activated() {
		return class_exists( 'FashionCore' ) ? true : false;
	}
}


if ( ! function_exists( 'fashion_get_metabox' ) ) {
	 ...
	 * @return bool|mixed
	 */
	function fashion_get_metabox( $id, $key, $default = false ) {
...
if ( ! function_exists( 'fashion_is_blog_archive' ) ) {
	function fashion_is_blog_archive() {
		return ( is_home() && is_front_page() ) || is_category() || is_tag() || is_post_type_archive( 'post' );
	}
}

if ( ! function_exists( 'fashion_get_excerpt' ) ) {
	function fashion_get_excerpt( $excerpt_length = 55 ) {
		global $post;
...
if ( ! function_exists( 'fashion_do_shortcode' ) ) {
	 ...
	 */
	function fashion_do_shortcode( $tag, array $atts = array(), $content = null ) {
...
if ( ! function_exists( 'fashion_nav_menu_social_icons' ) ) {
...
add_filter( 'walker_nav_menu_start_el', 'fashion_nav_menu_social_icons', 10, 4 );

if ( ! function_exists( 'fashion_social_links_icons' ) ) {
	 ...
	 */
	function fashion_social_links_icons() {
		...
		return apply_filters( 'fashion_social_links_icons', $social_links_icons );
	}
}

if ( ! function_exists( 'fashion_dropdown_icon_to_menu_link' ) ) {
	/**
	 ...
	 */
	function fashion_dropdown_icon_to_menu_link( $title, $item, $args, $depth ) {
...
add_filter( 'nav_menu_item_title', 'fashion_dropdown_icon_to_menu_link', 10, 4 );

if ( ! function_exists( 'fashion_is_header_builder' ) ) {
	/**
	 * @return bool
	 */
	function fashion_is_header_builder() {
		global $osf_header;
...
if ( ! function_exists( 'fashion_the_header_builder' ) ) {
	/**
	 * @return void
	 */
	function fashion_the_header_builder() {
		echo fashion_get_header_builder_html();
	}
}

if ( ! function_exists( 'fashion_get_header_builder_html' ) ) {

	/**
	 * @return string
	 */
	function fashion_get_header_builder_html() {
...
if ( ! function_exists( 'fashion_is_footer_builder' ) ) {
	/**
	 * @return bool
	 */
	function fashion_is_footer_builder() {
		global $osf_footer;
...
if ( ! function_exists( 'fashion_the_footer_builder' ) ) {
	/**
	 * @return void
	 */
	function fashion_the_footer_builder() {
		echo fashion_get_footer_builder_html();
	}
}


if ( ! function_exists( 'fashion_get_footer_builder_html' ) ) {
	function fashion_get_footer_builder_html() {
...
if ( ! function_exists( 'fashion_license_get_option' ) ) {
	function fashion_license_get_option( $key = '', $default = false ) {
...
function fashion_social_share() {
	if ( fashion_is_osf_framework_activated() && get_theme_mod( 'osf_socials' ) ) {
		$template = WP_PLUGIN_DIR . '/fashion-core/templates/socials.php';
...
function fashion_social_heading() {
	return esc_html__( 'Share:', 'fashion' );
}

add_filter( 'osf_social_heading', 'fashion_social_heading' );

function fashion_get_post_link( $taxonomy = 'category', $post_type = [ 'any' ] ) {
...
if ( ! function_exists( 'fashion_fnc_related_post' ) ) {
	function fashion_fnc_related_post( $relate_count = 4, $posttype = 'post', $taxonomy = 'category' ) {
		...
		if ( $related->have_posts() ) {
			...
			echo '<h3 class="related-heading">' . esc_html__( 'Related Articles', 'fashion' ) . '</h3>';
...
if (!function_exists('fashion_is_woocommerce_activated')) {
    function fashion_is_woocommerce_activated() {
        return class_exists('WooCommerce') ? true : false;
    }
}

if ( ! function_exists( 'fashion_get_review_counting' ) ) {
	function fashion_get_review_counting() {
